...
public class AsyncExportExcel {
	...
	public void exportExcel(String sheetName, BaseParam baseParam, OutputStream out) throws Exception {
		...
		for (int i = 0; i < sheetNum; i++) {
			...
			new Thread(new ExcelRunnable(countDownLatch,sheetName+(i+1), baseParam, startIndex, endIndex)).start();;
	...
	public class ExcelRunnable implements Runnable{
		...
		public ExcelRunnable(CountDownLatch countDownLatch, String sheetName, BaseParam baseParam, Integer startIndex,
				Integer endIndex) {
		...
		public void run() {
			try {
				...
				System.out.println("导出而excel出现错误");
				e.printStackTrace();
			}
